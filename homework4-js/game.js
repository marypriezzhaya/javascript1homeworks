...
var archive = [] // сведения о попытках
...
function guessNumber(){
	...
	while(gameIsRunning){
		...
		// игрок ввел некорректные данные
			...
			var move = prompt('какой ход хотите посмотреть?')
			alert(archive[move]);
			result = prompt("Введите число (-1 - закончить игру, -2 - посмотреть архив)", 0);
		...
		// проверяем число
			...
			if(answer[0]){
				...
				// следующий ход
				result = prompt("Быки: " + answer[1] + " Коровы: " + answer[2] + " Введите число (-1 - закончить игру, -2 - посмотреть архив)", 0);
			}
		}
	}
}
...
function checkNumber(myresult){
	archive[attempts + 1] = myresult;
	console.log(archive[attempts]);
